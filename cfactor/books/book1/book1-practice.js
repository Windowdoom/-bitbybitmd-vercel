...
/* ============== PRACTICE RENDERER ============== */
function getPScores(){try{return JSON.parse(localStorage.getItem('book1_practice_v1'))||{};}catch(e){return {};}}
function setPScore(id,pct){try{var d=getPScores();d[id]=pct;localStorage.setItem('book1_practice_v1',JSON.stringify(d));}catch(e){}}

window.renderBook1Practice=function(id){
 var P=window.BOOK1_PRACTICE||[], p=null;
 ...
 h+='<div class="cnav-foot"><button class="markbtn" onclick="window.checkBook1Practice(\''+p.id+'\')">CHECK ANSWERS</button><a class="btn-o" href="#ch'+p.ch+'">Back to Ch '+p.ch+'</a></div>';
...
window.checkBook1Practice=function(id){
 var P=window.BOOK1_PRACTICE||[], p=null;
 for(var i=0;i<P.length;i++){if(P[i].id===id)p=P[i];}
 if(!p)return;
 var esc=function(s){return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');};
...
(function(){
 ...
 window.addEventListener('hashchange',function(){
  ...
  if(m){window.renderBook1Practice(m[1]);}
 });
 var initial=location.hash||'';
 var im=initial.match(/practice-([\w-]+)/);
 if(im){setTimeout(function(){window.renderBook1Practice(im[1]);},100);}
})();
