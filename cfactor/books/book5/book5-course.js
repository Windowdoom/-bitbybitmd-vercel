/* Book 5 - Psychology & Sociology (Mind and Society) · course content + renderer.
   Ported from the BBB Book PsychSoc PDF (355pg).
   Section-level progress, sticky right-rail TOC, resume, inline quick-checks. */

/* ---------- MODULE GROUPING (left sidebar) ---------- */

var BOOK5_ROADMAP = { sections: [] };

var MODULES=[
 {name:'Biobehavioral', chs:[1,2,3],    ca:'var(--purple)', desc:'Biological foundations, sensation, and learning.'},
 {name:'Mind',          chs:[4,5,6],    ca:'var(--blue)',   desc:'Cognition, consciousness, motivation, and emotion.'},
 {name:'Development',   chs:[7,8,9],    ca:'var(--teal)',   desc:'Lifespan development, personality, and disorders.'},
 {name:'Social',        chs:[10,11,12], ca:'var(--gold)',   desc:'Social cognition, influence, and social structure.'},
 {name:'Society+Health',chs:[13],       ca:'var(--red)',    desc:'Inequality, demographics, and health.'}
];
...
var DONE_KEY='book5_done_v1';   /* section-level: {"0.1":true, "1.4":true, ...} */
var LAST_KEY='book5_last_v1';   /* {ch:N, sec:"x.y"} */
...
function totalSections(){var c=window.BOOK5_COURSE,n=0;for(var i=1;i<c.length;i++)n+=c[i].sections.length;return n;}
...
function buildNav(){
 var C=window.BOOK5_COURSE, html='', curMod=null;
...
function renderHome(){
 setActive('nav-home');
 var C=window.BOOK5_COURSE, done=getDone(), last=getLast();
 var nSec=totalSections(), dSec=doneSections();
 document.getElementById('cmob-title').textContent='Book 5 · Psych/Soc - Course Overview';
 ...
 h+='<h1 class="ph-title">Mind and Society<br><em>MCAT Psychology &amp; Sociology</em>, mechanism-first.</h1>';
 h+='<p class="ph-sub">Book 5 of the C-Factor Series. Biological foundations of behavior, sensation, learning, cognition, development, personality, disorders, and social science, taught mechanism-first. Section-level progress saves automatically. The PDF version is $40 (founders price; was $65) if you want the artifact. The content here is free, forever.</p>';
 ...
 h+='</div>';
 if(window.BOOK5_ROADMAP&&window.BOOK5_ROADMAP.sections){
  window.BOOK5_ROADMAP.sections.forEach(function(s,si){
   var open=si===0?' open':'';
 ...
 h+='<div class="ov-stats">'+
    ...
    '<div class="ov-stat"><b>355</b><span>pg source</span></div>'+
...
function render(n,secId){
 var C=window.BOOK5_COURSE, c=C[n];
 ...
 var h='';
 h+='<div class="crumb"><a href="#home">Book 5 · Psych/Soc</a> <span>/</span> '+esc(part)+(c.isScaffold?' <span style="color:var(--purple)">/ scaffold</span>':'')+'</div>';
 h+='<div class="cmeta">'+
...
window.toggleSec=function(id,n){
 ...
 var C=window.BOOK5_COURSE, c=C[n], st=chSectionStatus(c);
...
document.addEventListener('keydown',function(e){
 ...
 if(e.key==='ArrowRight'&&n<window.BOOK5_COURSE.length-1)location.hash='ch'+(n+1);
 if(e.key==='ArrowLeft'){if(n>1)location.hash='ch'+(n-1);else location.hash='home';}
});
function route(){
 if(!window.BOOK5_COURSE)return;
 ...
 if(n<1||n>window.BOOK5_COURSE.length)n=1;
 render(n,secId);
}
function init(){if(!window.BOOK5_COURSE){setTimeout(init,40);return;}buildNav();route();}
